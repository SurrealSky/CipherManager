...
BEGIN_MESSAGE_MAP(CMainFrame, CPIMDIFrameWndEx)
	...
	ON_MESSAGE(WM_LOAD_CHILDWIN, &CMainFrame::OnLoadChildwin)
	ON_WM_DROPFILES()
	ON_WM_KILLFOCUS()
	ON_WM_SETFOCUS()
END_MESSAGE_MAP()
...
afx_msg LRESULT CMainFrame::OnLoadChildwin(WPARAM wParam, LPARAM lParam)
{
	PDWORD data = (PDWORD)lParam;
	CRuntimeClass *pThis = (CRuntimeClass*)(*data);
	DWORD resourceid = *(data + 1);
	HINSTANCE mChildInstance = (HINSTANCE)wParam;
	HINSTANCE save_hInstance = AfxGetResourceHandle();
	AfxSetResourceHandle(mChildInstance);
	this->CreateNewChild(
		pThis, resourceid);
...
{
	CPIMDIFrameWndEx::OnKillFocus(pNewWnd);
...
{
	CPIMDIFrameWndEx::OnSetFocus(pOldWnd);
